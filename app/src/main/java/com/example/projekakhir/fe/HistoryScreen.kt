...
fun HistoryScreen(
    ...
    Scaffold(topBar = { TopAppBar(title = { Text("Riwayat & Pickup") }) }) { padding ->
        if (items.isEmpty()) {
...
fun HistoryCard(item: ItemModel) {
    Card(modifier = Modifier.fillMaxWidth()) {
        Column(modifier = Modifier.padding(16.dp)) {
            ...
            Text("Jenis: ${item.jenis} • Status: ${item.status}", style = MaterialTheme.typography.bodySmall, color = MaterialTheme.colorScheme.outline)
        }
    }
}
